...
pub fn read_confined_import_file(path: &Path, spec: &ConfinedImportSpec) -> Result<Vec<u8>, String> {
...
mod tests {
    ...
    fn confined_import_reads_respect_each_specs_extensions_and_cap() {
        ...
        let error =
            read_confined_import_file_within(&oversized, &folder, &TINY_JSONL).unwrap_err();
