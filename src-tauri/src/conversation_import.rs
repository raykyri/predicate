...
static STAGED_ARCHIVE: LazyLock<Mutex<Option<StagedArchive>>> =
...
fn detect_archive_format(conversations: &[Value]) -> Result<ImportArchiveFormat, String> {
    ...
    let format = format_of(&conversations[0]).ok_or_else(|| {
        "unrecognized export format: expected a claude.ai or ChatGPT conversations.json"
            .to_string()
    })?;
    if conversations
        .iter()
...
mod tests {
    ...
    fn claude_code_session_listing_scans_projects_with_previews_and_cwd() {
        ...
        let subagents = project.join("11111111-aaaa-bbbb-cccc-000000000001").join("subagents");
    ...
    fn opencode_session_assembly_orders_messages_and_attaches_parts() {
        ...
        let _ = fs::remove_dir_all(&storage);
        write_opencode_store(&storage);
        let metadata_path = storage.join("session").join("global").join("ses_alpha.json");
        ...
        assert_eq!(messages[0]["parts"][0]["text"], "trim the changelog to the last release");
        ...
        let empty = storage.join("session").join("global").join("ses_empty.json");
        ...
        fs::write(&outside, serde_json::json!({ "id": "ses_alpha" }).to_string()).unwrap();
