...
/// A single record serialized larger than this is logged without its data, as
/// a marker noting the dropped size. A sane record (a stall report with its
/// breadcrumb slice) is a few KB; anything near this cap is a runaway payload,
/// and rotation checks the file length only between appends, so one oversized
/// line would blow straight through LOG_MAX_BYTES.
    ...
    fn append_to_log(&self, record: &DiagnosticRecord) {
        ...
        if line.len() > MAX_RECORD_BYTES {
            let marker = DiagnosticRecord {
                ...
                message: format!(
                    "{} [data dropped: {} byte record exceeded the per-record cap]",
                    record.message,
                    line.len()
                ),
                data: Value::Null,
            ...
            match serde_json::to_string(&marker) {
                Ok(marker_line) => line = marker_line,
                Err(_) => return,
            }
        }
...
mod tests {
    ...
    #[test]
    fn oversized_records_are_logged_as_markers_without_data() {
        let dir = std::env::temp_dir().join(format!("qmux-diag-cap-{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        let log_path = dir.join("diagnostics.log");
        let diagnostics = Diagnostics::with_log(log_path.clone(), u64::MAX);
        let huge = "x".repeat(MAX_RECORD_BYTES + 1);
        diagnostics.record("backend", "test.huge", "huge", json!({ "blob": huge }));
        let line = fs::read_to_string(&log_path).expect("log written");
        assert!(
            line.len() < MAX_RECORD_BYTES,
            "oversized record must not reach the log verbatim"
        );
        let parsed: DiagnosticRecord =
            serde_json::from_str(line.lines().next().expect("one line")).expect("parseable");
        assert_eq!(parsed.category, "test.huge");
        assert!(parsed.message.contains("data dropped"));
