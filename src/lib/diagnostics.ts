...
export class DiagnosticsBuffer {
  ...
  report(category: string, message: string, data?: unknown): void {
    // The ring copy must not carry an attached breadcrumb slice: reports
    // embedding earlier reports' breadcrumbs would nest recursively, and the
    // payload grows exponentially with each stall (a stall report's serialize
    // + IPC cost then *causes* the next, longer stall).
    this.breadcrumb(category, message, withoutBreadcrumbs(data));
...
/** Report data with any `breadcrumbs` attachment dropped (see report()). */
function withoutBreadcrumbs(data: unknown): unknown {
  ...
  const { breadcrumbs: _dropped, ...rest } = data as Record<string, unknown>;
  return rest;
}
