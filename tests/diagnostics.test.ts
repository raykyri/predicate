...
test("stall reports never re-ingest earlier reports' breadcrumb slices", () => {
  // Regression: report() used to breadcrumb its full data, so each stall
  // report embedded the previous ones' breadcrumbs recursively — payloads
  // grew exponentially and serializing them caused the next, longer stall.
  let now = 0;
  const buffer = new DiagnosticsBuffer(() => now);
  const batches: FrontendDiagnosticEntry[][] = [];
  const timers = new Map<number, { handler: () => void; ms: number }>();
  ...
  for (let stalls = 0; stalls < 3; stalls += 1) {
    now += STALL_TICK_MS + 5_000;
    stallTimer.handler();
  }

  assert.equal(batches.length, 3);
  const last = batches[2].find((entry) => entry.category === "frontend.main_thread_stall");
  const lastData = last?.data as { breadcrumbs: FrontendDiagnosticEntry[] };
  const priorStalls = lastData.breadcrumbs.filter(
    (crumb) => crumb.category === "frontend.main_thread_stall",
  );
  assert.equal(priorStalls.length, 2, "earlier stalls still appear as breadcrumbs");
  for (const prior of priorStalls) {
    assert.ok(
      !("breadcrumbs" in (prior.data as Record<string, unknown>)),
      "a breadcrumbed stall report carries no breadcrumb slice of its own",
    );
  }
});
